...
function archived_events(){
  ...
  //upcoming events 
  $archived_events = "SELECT COUNT(id) AS count 
      FROM {$table} WHERE TimeEventStart < '". $current_date."%'";
  $events_rows = $wpdb->get_results($archived_events);
  ...
  if(isset($events_rows)){
    $total_pages = ceil(($events_rows[0]->count / $limit));
  }
  ...
  //get current events 
  $rs_events = "SELECT * 
    FROM {$table} WHERE TimeEventStart < '". $current_date."%' order by TimeEventStart DESC limit $start,$end";
...
<div class="wrap" id="wp-media-grid" data-search="">
  ...
  <p><span>Archived Events   (<?php echo $events_rows[0]->count;?>)</span></p>
</div>  
